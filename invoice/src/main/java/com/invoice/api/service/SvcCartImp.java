...
public class SvcCartImp implements SvcCart {
	...
	public ApiResponse addToCart(Cart cart) {
		...
		ResponseEntity<DtoProduct> response = productPl.getProduct(cart.getGtin());		
		product = response.getBody();	

		Integer product_stock = product.getStock(); // cambiar el valor de cero por el stock del producto recuperado de la API Product 
		
		if(cart.getQuantity() > product_stock) {
			throw new ApiException(HttpStatus.BAD_REQUEST, "Out of stock");
		}
		
		Cart cart1 = repo.findByRfcAndGtin(cart.getRfc(),cart.getGtin());
		
		if(cart1 != null){
			if(cart1.getQuantity()+cart.getQuantity() > product_stock) {
				throw new ApiException(HttpStatus.BAD_REQUEST, "invalid quantity");
			}
			try {
				repo.updateQuantity(cart.getRfc(), cart.getGtin(), cart.getQuantity()+cart1.getQuantity());
			}catch (DataIntegrityViolationException e) {
				throw new ApiException(HttpStatus.BAD_REQUEST, "couldn't update quantity");
			}
