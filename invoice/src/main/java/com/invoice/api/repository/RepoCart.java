...
public interface RepoCart extends JpaRepository<Cart, Integer>{
	...
	@Modifying
	@Transactional
	@Query(value ="UPDATE cart SET quantity =:quantity WHERE rfc =:rfc AND gtin =:gtin AND status = 1", nativeQuery = true)
	Integer updateQuantity(@Param("rfc") String rfc, @Param("gtin") String gtin, @Param("quantity") Integer quantity);
